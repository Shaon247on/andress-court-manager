...
import { Download, DollarSign, TrendingUp, Clock, Wallet, Loader2 } from 'lucide-react';
...
export default function RevenueOverview({ data, errorMessage }: RevenueOverviewProps) {
    ...
    <div className="h-full flex flex-col p-4 sm:p-6 lg:p-8 bg-white overflow-y-auto w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8 shrink-0">
        <div className="bg-white border border-slate-200 rounded-lg p-4 sm:p-5 flex flex-col items-start relative overflow-hidden">
          <div className="text-sm text-slate-500 font-medium mb-1">Total Earnings</div>
          <div className="text-2xl sm:text-3xl font-bold text-slate-900">${parseFloat(data.total_earnings).toLocaleString()}</div>
          <div className="text-xs text-slate-400 mt-1 font-medium">From {data.bookings_count} bookings</div>
          <DollarSign className="absolute top-4 right-4 w-5 h-5 text-emerald-500" />
        </div>

        <div className="bg-white border border-slate-200 rounded-lg p-4 sm:p-5 flex flex-col items-start relative overflow-hidden">
          <div className="text-sm text-slate-500 font-medium mb-1">Total Withdrawn</div>
          <div className="text-2xl sm:text-3xl font-bold text-teal-600">${parseFloat(data.total_withdrawn).toLocaleString()}</div>
          <div className="text-xs text-slate-400 mt-1 font-medium">
            {data.withdrawal_requests.filter(w => w.status === 'paid').length} processed withdrawals
          </div>
          <Wallet className="absolute top-4 right-4 w-5 h-5 text-teal-500" />
        </div>

        <div className="bg-white border border-slate-200 rounded-lg p-4 sm:p-5 flex flex-col items-start relative overflow-hidden">
          <div className="text-sm text-slate-500 font-medium mb-1">Pending Amount</div>
          <div className="text-2xl sm:text-3xl font-bold text-orange-500">${parseFloat(data.pending_amount).toLocaleString()}</div>
          <div className="text-xs text-slate-400 mt-1 font-medium">{data.pending_count} pending</div>
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-4 sm:p-5 flex flex-col items-start relative overflow-hidden">
          ...
          <div className="text-2xl sm:text-3xl font-bold text-purple-600">${parseFloat(data.available_balance).toLocaleString()}</div>
          <div className="text-xs text-slate-400 mt-1 font-medium">Ready to withdraw</div>
      ...
      <div className="bg-blue-50/30 border border-blue-100 rounded-xl p-4 sm:p-6 mb-8 shrink-0">
        <h3 className="text-base sm:text-lg font-bold text-slate-900 mb-4">Request Withdrawal</h3>
        ...
        <p className="text-sm text-slate-500 font-medium">
          Withdrawal requests are processed weekly on Mondays. Maximum withdrawal amount: ${parseFloat(data.available_balance).toLocaleString()}
        </p>
      </div>

      {/* Withdrawal Requests Table */}
      <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-4 shrink-0">Withdrawal Requests</h3>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 shrink-0 gap-2">
        <h3 className="text-lg sm:text-xl font-bold text-slate-900">Earnings Summary</h3>
        <Link href="/dashboard/revenue/earnings">
          <Button variant="primary" size="sm" className="h-9 px-5 rounded-md font-semibold text-sm w-full sm:w-auto">
            See All
          </Button>
        </Link>
      </div>
      ...
      <Dialog open={withdrawDialogOpen} onOpenChange={setWithdrawDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to withdraw <span className="font-bold">${withdrawAmount}</span>?
              This amount will be transferred to your default payout method.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
